...
[Query confidence toggles](../end-user/reference/query-confidence-toggles.md).
...
[Why clarifying questions appear](../end-user/explanation/query-confidence.md).
...
[Verify queries across models](../project-developer/verification.md).
...
drafts. See [Trusting AI-generated results](../end-user/explanation/trusting-ai-results.md)
for the posture we recommend when acting on them.

## Where to read more

- [Choosing an LLM](choosing-an-llm.md) — model selection and data
  sensitivity tradeoffs.
- [SQL result cache](sql-result-cache.md) — how repeat queries are
  served without re-running the model or the database.
- [Architecture](../tool-developer/architecture.md) — the code-level view
  of the same loop.
