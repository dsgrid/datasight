/** Fuzzy search scoring — ported from search_helpers.js */
...
export function scoreFuzzySubsequence(query: string, value: string): number {
...
export function scorePaletteResult(
  query: string,
  haystacks: string[],
  baseScore: number,
): number {
  ...
  for (const value of haystacks) {
    ...
    else if (lower.split(/[\s._-]+/).some((part) => part.startsWith(q)))
...
export interface SchemaEntry {
  ...
  matchingColumns: { name: string }[];
}

export function getVisibleSchemaEntries(
  ...
  return (tables || [])
    .map((table) => {
      const tableMatches = String(table.name || "")
        .toLowerCase()
